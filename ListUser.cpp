// ListUser.cpp : Defines the entry point for the console application.
//
...
void GetUserInfo(LPWSTR user)
{
	...
	while (i < 24)
	{
		...
		//
		dwLevel = i;
		wprintf(L"\nCalling NetUserGetinfo with Username=%s Level=%d\n", user, dwLevel);
		nStatus = NetUserGetInfo(NULL, user, dwLevel, (LPBYTE *)& pBuf);
		//
		...
		if ((nStatus == NERR_Success) || (nStatus == ERROR_MORE_DATA))
		{
			if ((pTmpBuf = pBuf) != NULL)
			{
				...
				for (i = 0; (i < dwEntriesRead); i++)
				{
					...
					wprintf(L"\t-- %s\n", pTmpBuf->usri0_name);
					//
					//  Print the user account's information
					//
					GetUserInfo(pTmpBuf->usri0_name);
					pTmpBuf++;
					dwTotalCount++;
				}
			}
